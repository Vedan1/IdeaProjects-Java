...
public class tester {
    public static void main(String[] args) throws DataFormatException, PenExceptionHandeling {
                ...
                try {
                    ...
                    switch (sc.nextInt()) {
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            System.out.println("Enter value to be deleted: Brand,Material");
                            String Brands = sc.next();
                            String Materi = sc.next();
                            for(Pen i : penList.values()){
                                if(i.getBrand().toString().equals(Brands)&&i.getMaterial().equals(Materi)){
                                    penList.values().remove(i);
                                    System.out.println("Value Removed Successfully");
                                                                    }
                            }
                                break;
                        case 7:
                            System.out.println("Sorting in ListingDate Ascending");
                            ArrayList<Pen> list = new ArrayList<>(penList.values());
                            Comparator<Pen> comp = Comparator.comparing(Pen::getStockListingDate);
                            Collections.sort(list,comp.reversed());
                            list.forEach(f -> System.out.println(f));

                            break;
                        case 0:
                            for (Pen f : penList.values()) {

                                if (f.getBrand().toString() == "CELLO") {
